...
var errorMsg = !1,
    mState = 0,
    connection = new WebSocket('ws://' + location.hostname + ':1880/ws/test');
var msgGrid;
...
connection.onmessage = function (e) {
    ...
    switch (m._state) {
        ...
        case 9:
        case 10:
        case 11: drawStatus(2);
            drawKeyPad();
            break;
        case 12: drawStatus(3);
            drawKeyPad();
            drawFire();
            break;
        case 13:
    ...
    switch (n) {
        ...
        case 3:
            if (9 <= pdata._state && pdata._state <= 11) {
                document.getElementById('status-text').innerHTML = 'Aiming...'
            } else {
                var statusIcon = [],
                    statusText = '';
                ...
                switch (pdata._keyedPreset % 3) {
                    case 1: statusIcon.push('left');
                        statusText += (! pdata._hand ? 'Inside' : 'Outside');
                        break;
                    case 2: statusText += 'Mid';
                        break;
                    case 0: statusIcon.push('right');
